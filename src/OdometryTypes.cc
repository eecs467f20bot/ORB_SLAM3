...
IntegratedRotation::IntegratedRotation(const double & delta_L, const double & delta_R, const Bias &odomBias, const float &time):
    deltaT(time)
{
    const float x = 0;
    const float y = 0;
    ...
    const float B = 0.11;
    const float k = 2*3.14*0.042 / (20*78);
    const float theta = ((delta_R-odomBias.bR) - (delta_L-odomBias.bL)) * k / B;
    const float z = (theta)*time;
...
void Preintegrated::Reintegrate()
{
    ...
    for(size_t i=0;i<aux.size();i++)
        IntegrateNewMeasurement(aux[i].delta_L,aux[i].delta_R,aux[i].t);
}

void Preintegrated::IntegrateNewMeasurement(const float &delta_L, const float &delta_R, const float &dt)
{
    mvMeasurements.push_back(integrable(delta_L,delta_R,dt));
    ...
    float B = 0.11;
    float k = 2*3.14*0.042 / (20*78);
    float theta = ((delta_R-odomBias.bR) - (delta_L-odomBias.bL)) * k / B;
    float d = ((delta_R-odomBias.bR) + (delta_L-odomBias.bL)) * k / 2;
    cv::Mat ds = (cv::Mat_<float>(3,1) << d*cos(theta), d*sin(theta), 0);
    // Update delta position dP and velocity dV (rely on no-updated delta rotation)
    dP = dP + dR*ds;

    // cv::Mat vel = (cv::Mat_<float>(3,1) << s*cos(theta)/dt, s*sin(theta)/dt, 0);
    dV = dV + dR*ds/dt;

    cv::Mat acc = vel/dt;
    cv::Mat accW = (cv::Mat_<float>(3,1) << 0, 0, theta/dt);
    ...
    // TODO: Update position and velocity jacobians wrt bias correction
    ...
    Jthetal = k/B;
    JthetaR = -k/B;
    Jsl = (cv::Mat_<float>(3,1) << k/2*cos(theta) + d*sin(theta)*k/B, k/2*sin(theta) + d*cos(theta)*k/B, 0);
    Jsr = (cv::Mat_<float>(3,1) << k/2*cos(theta) - d*sin(theta)*k/B, k/2*sin(theta) - d*cos(theta)*k/B, 0);
    JPl = JPl + dR*Jsl;
    JPr = JPr + dR*Jsr;
    JVl = JVl + dR*Jsl/dt;
    JVr = JVr + dR*Jsr/dt;
    ...
    IntegratedRotation dRi(delta_L, delta_R, b, dt):
    dR = NormalizeRotation(dR*dRi.deltaR);

    // TODO: Compute rotation parts of matrices A and B
    ...
    // TODO: Update rotation jacobian wrt bias correction
...
void Preintegrated::MergePrevious(Preintegrated* pPrev)
{
    ...
    for(size_t i=0;i<aux1.size();i++)
        IntegrateNewMeasurement(aux1[i].delta_L,aux1[i].delta_R,aux1[i].t);
    for(size_t i=0;i<aux2.size();i++)
        IntegrateNewMeasurement(aux2[i].delta_L,aux2[i].delta_R,aux2[i].t);
...
void Preintegrated::SetNewBias(const Bias &bu_)
{
    ...
    db.at<float>(0) = bu_.bL-b.bL;
    db.at<float>(1) = bu_.bR-b.bR;
}

odom::Bias Preintegrated::GetDeltaBias(const Bias &b_)
{
    std::unique_lock<std::mutex> lock(mMutex);
    return odom::Bias(b_.bL-b.L,b_.bR-b.bR);
}
